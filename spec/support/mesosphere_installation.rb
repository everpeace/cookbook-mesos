...
shared_examples_for 'an installation from mesosphere' do |opt|
  let :mesos_deb do
    chef_run.remote_file(File.join(Chef::Config[:file_cache_path], 'mesos_0.16.0.deb'))
  end
  ...
  it 'downloads mesos deb' do
    expect(chef_run).to create_remote_file File.join(Chef::Config[:file_cache_path], 'mesos_0.16.0.deb')
  end
